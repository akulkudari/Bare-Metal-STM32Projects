...
void init_PB6and7_asI2C(void) {
    ...
    GPIOB->OSPEEDR |= (0x1 << (6 * 2)); //set OSPEEDR to Medium for I2C
    // Pullup
    GPIOB->PUPDR &= ~(0x3 << (6*2));
    GPIOB->PUPDR |= (0x1 << (6*2));
    ...
    GPIOB->OSPEEDR |= (0x1 << (7 * 2)); //set OSPEEDR to Medium for I2C
    // Pullup
    GPIOB->PUPDR &= ~(0x3 << (7*2));
    GPIOB->PUPDR |= (0x1 << (7*2));
}
...
typedef struct{
    ...
    * Bit 9 - START Generation: 
    * Bit 15 - SWRST: 
    *   -When set to 1, I2C is under reset.
    *   -Note: Make sure I2C lines are released and bus is free 
    */
